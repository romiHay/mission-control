...
const GenericSelect: React.FC<{
    ...
    >
        <option value="">{placeholder} (ללא שינוי)</option>
        {options.map(opt => (
...
const BulkEditRuleForm: React.FC<BulkEditRuleFormProps> = ({ rules, geometries, missionName, onClose, onSave, darkMode }) => {
    ...
    useEffect(() => {
        ...
        m.invalidateSize();
        const timer = setTimeout(() => m.invalidateSize(), 300);
    ...
    useEffect(() => {
        ...
        if (bounds.isValid() && !hasFittedRef.current && geometries.length > 0) {
    ...
    useEffect(() => {
        ...
        selectedRuleIds.forEach(id => {
            ...
            const geo = geometries.find(g => g.id === rule?.geometryId || g.ruleId === id);
            if (geo) {
                selectedCount++;
                lastGeo = geo;
                if (geo.type === 'Point') {
        ...
        if (bounds.isValid()) {
            if (selectedCount === 1 && lastGeo && lastGeo.type === 'Point') {
                map.flyTo(lastGeo.coordinates as [number, number], 18, { duration: 1 });
            } else {
                map.flyToBounds(bounds, { padding: [60, 60], duration: 1, maxZoom: 18 });
            }
        }
    ...
    return (
        <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4 md:p-10 animate-fadeIn" dir="rtl">
            ...
            <div className="relative w-full max-w-5xl max-h-[90vh] bg-white dark:bg-slate-900 shadow-2xl rounded-[2.5rem] flex flex-col overflow-hidden border border-gray-100 dark:border-slate-800 animate-slideUp">

                <header className="p-8 border-b border-gray-100 dark:border-slate-800 flex justify-between items-center bg-white/50 dark:bg-slate-900/50 backdrop-blur-md sticky top-0 z-10 font-heebo">
                    <div className="flex flex-col">
                        <label className="text-[10px] font-black text-indigo-500 uppercase tracking-[0.2em] mb-1">פעולות קבוצתיות</label>
                        <h2 className="text-2xl font-black text-gray-800 dark:text-white leading-none">עריכת חוקים מרובה</h2>
                    </div>
                    <button onClick={onClose} className="p-3 hover:bg-gray-100 dark:hover:bg-slate-800 rounded-2xl transition-all active:scale-90 text-gray-400">
                ...
                <div className="flex-1 overflow-y-auto p-8 custom-scrollbar">
                    <div className="grid grid-cols-1 md:grid-cols-5 gap-10">
                        ...
                        <div className="md:col-span-2 space-y-6 flex flex-col min-h-0">
                            <div className="space-y-4 flex flex-col h-full">
                                <div className="flex justify-between items-center shrink-0">
                                    <label className="text-[11px] font-black text-gray-400 dark:text-slate-500 uppercase tracking-widest font-heebo">בחירת חוקים ({selectedRuleIds.length})</label>
                                    <button
                                        onClick={handleSelectAll}
                                        className="text-[10px] font-bold text-indigo-600 dark:text-indigo-400 hover:underline font-heebo"
                                    >
                                    ...
                                    {rules.map(rule => (
                                        ...
                                        >
                                            ...
                                            <div className="flex-1 overflow-hidden">
                                                <p className={`font-bold text-sm truncate font-heebo ${selectedRuleIds.includes(rule.id) ? 'text-indigo-900 dark:text-indigo-100' : 'text-gray-700 dark:text-slate-300'}`}>
                                                    {rule.name}
                                                </p>
                        ...
                        <div className="md:col-span-3 space-y-8">
                            ...
                            <div className="p-6 bg-amber-50 dark:bg-amber-900/10 rounded-[2rem] border border-amber-100 dark:border-amber-800/30 font-heebo">
                ...
                <footer className="p-8 border-t border-gray-100 dark:border-slate-800 bg-gray-50/50 dark:bg-slate-800/20 flex gap-4 shrink-0 font-heebo">
                    <button
                        onClick={onClose}
